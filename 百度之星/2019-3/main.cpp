...
void dijkstra(int s)
{
    ...
    for(int i=0;i<=n;i++) d[s][i]=make_pair(inf,0);
    ...
    for(int i=head[s];i;i=e[i].next){
        int x=e[i].go;
        d[s][x].first=min(d[s][x].first,e[i].w);
        cout<<"!!!"<<d[s][x].first<<endl;
        d[s][x].second=0;
        q.push(make_pair(d[s][x],x));
    }
    while(!q.empty()){
        int x=q.top().second;q.pop();//cout<<"----------x="<<x<<' '<<d[s][x].first<<' '<<d[s][x].second<<endl;
        if(v[x])continue;v[x]=1;
        for(int i=head[x];i;i=e[i].next) {
            int y=e[i].go;
            //cout<<"x="<<x<<",y="<<y<<endl;
            if (d[s][x].first + e[i].w < d[s][y].first || d[s][x].second < d[s][y].second) {
                d[s][y].first = d[s][x].first + e[i].w;
                d[s][y].second = max(d[s][x].second, x);
                //cout<<s<<' '<<x<<' '<<y<<endl;
...
int main()
{
    ...
    while(tt--){
        ...
        F(i,1,n) F(j,1,n) ans=(ans+d[i][j].second)%mod;
        cout<<ans<<endl;
    }
    return 0;
}
