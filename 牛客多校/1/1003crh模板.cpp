...
struct ChairmanTree {
    ...
    void debug(int p, int l, int r) {
        cout<<"~~"<<p<<' '<<sum[p]<<' '<<ls[p]<<' '<<rs[p]<<endl;//sum[p] = 0;
        if (l == r)return;
        int mid = (l + r) >> 1;
        //cout<<"***"<<ls[p]<<' '<<rs[p]<<endl;
        debug(ls[p], l, mid);
...
int main()
{
    ...
    while(tt--){
        ...
        for(int i=0;i<n;i++)
            T.update(T.root[i+1],T.root[i],1,n,A.sa[i]+1),cout<<"**"<<A.sa[i]+1<<endl;
        F(i,0,n) cout<<T.root[i]<<' ';cout<<endl;
        F(i,1,n) cout<<"——"<<T.root[i]<<"——"<<endl,T.debug(T.root[i],1,n);
        while(q--){
