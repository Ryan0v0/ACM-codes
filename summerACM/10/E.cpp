...
double work()
{
    ...
    for(int i=2;i<=n;i++)
    {
        R[i]=dis(p[i],p[i-1])-R[i-1];
        if(dcmp(R[i])<0)return INF;
        S+=sqr(R[i]);
    }
    if(dcmp(R[n]+R[1]-dis(p[1],p[n])))return INF;
    return S;
}
int main()
{
    ...
    while(T--)
    {
        ...
        double l=0,r=min(dis(p[1],p[2]),dis(p[1],p[n])),resl,resr,sum=0;
        for(int i=1;i<=n;i++)
        {
        ...
        while(r-l>eps)
        {
            ...
            R[1]=mid1;double res1=work();
            R[1]=mid2;double res2=work();
            if(dcmp(res1-res2)<0)r=mid2,resr=res2;
            else if(dcmp(res1-res2)>0)l=mid1,resl=res1;
            else if(dcmp(res1-INF))r=mid2,resr=res2;
            else l=mid1,resl=res1;
        }
        assert(dcmp(resl-INF)||dcmp(resr-INF));
